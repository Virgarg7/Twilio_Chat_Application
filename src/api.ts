...
export async function addConversation(
  ...
  try {
    console.log("************in Try **************",client);
    const conversation = await client.createConversation({friendlyName: name});
    console.log("____in conv___", conversation)
    await conversation.join();

    const participants = await conversation.getParticipants();
    console.log("____in participants_________", conversation)
    ...
    //   console.log("***********************************************",conv);
    //   await conv.join();
    //   const part = await conv.getParticipants();
    //   updateParticipants(part, conv.sid);
    //   successNotification({
    //       message: CONVERSATION_MESSAGES.CREATED,
    //       addNotifications,
    //     })
    ...
    //     console.log("in catch the bitch",error);
    //     return error
    // })
    // await conversation.join();
...
export async function addNonChatParticipant(
  ...
  try {
    const result = await convo.addNonChatParticipant('+14175386333', number, {
      friendlyName: number,
    });
    successNotification({
      message: PARTICIPANT_MESSAGES.ADDED,
...
export async function getToken(
  ...
  try {
    ...
    // console.log("_____log____in____", error);
